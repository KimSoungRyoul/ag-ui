...
//!
//! - [`HistoryMessage`] from an [`ag_ui::Message`], so the surface-recovery
//!   scan can read an AG-UI thread directly.
//! - [`find_prior_surface_in`], the same scan without the mapping step.
//! - [`ag_ui::Tool`] from a [`ToolDefinition`], so the toolkit's two tool
...
/// [`crate::toolkit::history`] scans on. User and tool multimodal parts are
/// flattened to their text: an A2UI envelope is never an image.
